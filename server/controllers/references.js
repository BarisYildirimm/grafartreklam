...
export const createdReference = async (req, res) => {
  const s3 = new aws.S3({
  ...
  try {
    const uploadSingle = upload("grafart-reference-image").single(
      "referenceImage"
    );
...
export const updateReference = async (req, res) => {
  ...
  try {
    const updateSingle = update("grafart-reference-image").single(
      "referenceImage"
    );
    updateSingle(req, res, async (err) => {
      ...
      if (!mongoose.Types.ObjectId.isValid(_id))
        return res.status(404).send("ID'ye sahip Referans yok!");
...
export const deleteReference = async (req, res) => {
  ...
  const imageId = await Reference.find(
  ...
  s3.deleteObjects(
    {
      Bucket: "grafart-reference-image",
      Delete: {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).json({ message: "ID'ye sahip Referans yok!" });
